...
public class GaussianBlur {
	...
	public BufferedImage createGaussianedImage(BufferedImage source_image, double weights[][], int radius) {
		...
		for(int x = 0; x < source_image.getWidth(); x ++)
		{
			for(int y = 0; y < source_image.getHeight(); y ++)
			{
				...
				answer.setRGB(x, y, new Color(getWeightedColorValue(distributedColorRed), getWeightedColorValue(distributedColorGreen), getWeightedColorValue(distributedColorBlue) ).getRed());
			}
		}
		return answer;
	}
